...
export const generateA4InvoiceHTML = (sale: any, settings: any) => {
  ...
  const safeData = {
    ...
    customer: {
      name: sale?.customer?.name || sale?.customerDetails?.name || sale?.customerName || sale?.selectedCustomer?.name || '',
      address: sale?.customer?.address || sale?.customerDetails?.address || sale?.customerAddress || sale?.selectedCustomer?.address || '',
      phone: sale?.customer?.phone || sale?.customerDetails?.phone || sale?.customerPhone || sale?.selectedCustomer?.phone || '',
    },
